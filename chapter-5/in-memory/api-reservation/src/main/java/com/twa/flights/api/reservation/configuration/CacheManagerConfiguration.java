...
public class CacheManagerConfiguration {
    ...
    //Define a bean with all the caches, and which is the method responsible to obtain the value if not exist in the cache
    @Bean
    public CacheManager cacheManager() {
        CacheSettings cacheCitySettings = cacheConfiguration.getCacheSettings(CATALOG_COUNTRY);

        SimpleCacheManager simpleCacheManager = new SimpleCacheManager();
        simpleCacheManager.setCaches(List.of(buildCaffeineCache(CATALOG_COUNTRY, cacheCitySettings, catalogConnector::getCountry)));
